...
class TeamApi extends BaseApi implements ITeamApi {
  ...
  Future<List<String>> invite(
    ...
    final invitations = invitationTeamModel.invitations
        ...
        .toList();
    final body = invitationTeamModel.cid != null && invitationTeamModel.cid!.isNotEmpty ? {
  ...
  Future<InvitationPendingAcceptedWrappedModel> getPendingAcceptedInvitations(
    ...
    final res = await _networkHandler.get(
        path:
            "${Endpoint.teams}/$teamId/invitations?q=$query&status=$status&offset=$offset&max=$max&dateorder=$dateOrder");
    if (res.statusCode == RemoteConstants.code_success) {
  ...
  Future<SearchTasksModel> searchTasks(
      String teamId,{int max = 100, int offset = 0, String query = "", ValueChanged<CancelToken>? onCancelToken}) async {
    final res = await _networkHandler.get(
        path:
            "${Endpoint.teams}/$teamId/search?entity=tasks&offset=$offset&max=$max&q=$query", onCancelToken: onCancelToken);
    if (res.statusCode == RemoteConstants.code_success) {
  ...
  Future<SearchFilesModel> searchFiles(
    ...
    final res = await _networkHandler.get(
        path:
            "${Endpoint.teams}/$teamId/search?entity=files&offset=$offset&max=$max&q=$query", onCancelToken: onCancelToken);
    if (res.statusCode == RemoteConstants.code_success) {
      SearchFilesModel model = _iTeamConverter.fromJsonSearchFiles(jsonDecode(res.data));
      return model;
    }
    throw serverException(res);
  }
  ...
  Future<SearchMembersModel> searchMembers(
    ...
    final res = await _networkHandler.get(
        path:
            "${Endpoint.teams}/$teamId/search?active=true&entity=members&offset=$offset&max=$max&q=$query", onCancelToken: onCancelToken);
    if (res.statusCode == RemoteConstants.code_success) {
  ...
  Future<SearchMessagesModel> searchMessages(
    ...
    final res = await _networkHandler.get(
        path:
            "${Endpoint.teams}/$teamId/search?entity=messages&offset=$offset&max=$max&q=$query", onCancelToken: onCancelToken);
    if (res.statusCode == RemoteConstants.code_success) {
  ...
  Future<bool> checkTeamNameInUse(String teamName) async {
    final res = await _networkHandler.head(path: "${Endpoint.teams}/$teamName", useVersionInUrl: true);
  ...
  Future<WhiteBrandConfig> getWhiteBrandConfig(String domain) async {
    final res = await _networkHandler.get(path: "${Endpoint.public}${Endpoint.enterprise}/$domain");
    if (res.statusCode == RemoteConstants.code_success) {
