...
class TXMessageBodyWidget extends StatelessWidget {
  ...
  Widget _getMessageHtmlContent(MessageModel messageModel) {
    ...
    final linksData =
        messageModel.extra.where((element) => element.type.trim().toLowerCase() == MessageExtraTypes.link.name).toList();
    return (msg.trim().isEmpty)
        ? Container()
        : Column(
      crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              messageModel.isAnswerMessage
                  ? Container(
                      child: TXAnswerMessageWidget(
      ...
      children: [
        ...extras
            .map((e) => Container(
                  child: InkWell(
                    onTap: () async {
                      if(e.type.trim().toLowerCase() == MessageExtraTypes.link.name) {
                        if(e.url?.isNotEmpty == true && (await canLaunchUrlString(e.url!))) {
                          launchUrlString(e.url!);
                        } else if (e.providerUrl?.isNotEmpty == true && (await canLaunchUrlString(e.providerUrl!))) {
                          launchUrlString(e.providerUrl!);
                        }
                      } else if(e.type.trim().toLowerCase() == MessageExtraTypes.rich.name) {}
                    },
                    child: Card(
                                ...
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      TXTextWidget(
                                          text:
                                              e.providerName?.isNotEmpty == true
                                                  ? e.providerName!
                                                  : R.string.noTitle,
                                          color: R.color.secondaryColor,
                                          fontWeight: FontWeight.bold),
